...
public class GameMap {
    ...
    private void initRegisterBombsAndSwitches() {
        ...
        List<Activator> activators = getEntities(Activator.class);
        // for (LogicalEntity le : logicalEntities) {
        // for (Activator a : activators) {
        // if (Position.isAdjacent(le.getPosition(), a.getPosition())) {
        // le.subscribe(a);
        // a.subscribe(le);
        // }
        // }
    ...
    public Boolean checkAllSpawnerDestroyed() {
        List<ZombieToastSpawner> zombieToastSpawners = getEntities(ZombieToastSpawner.class);
        return zombieToastSpawners.size() < 1;
    }

    public Boolean checkZombieOnMap() {
        List<ZombieToast> zombieToast = getEntities(ZombieToast.class);
        return zombieToast.size() >= 1;
    }
}
